...
def opportunist_noise(theta):
    if theta > 4.0:
        return random.uniform(0.0, 0.2)   # trying to win trust back
    else:
        return random.uniform(0.2, 0.8)   # cheat more when scrutiny is low

def chaotic_noise():
    return random.uniform(0.0, 1.0)       # unpredictable

def drifter_noise(step):
    # Slowly drift pattern over time: sinusoidal shift
...
# ----------------------------------------------------------
personality_types = ["HONEST", "ATTACKER", "OPPORTUNIST", "CHAOTIC", "DRIFTER"]
...
def evolve_persona(agent, step):
    ...
    if p == "ATTACKER" and step % 100 == 0:
        ...
        agent["persona"] = random.choice(personality_types)


# ----------------------------------------------------------
# Get noise based on persona
# ----------------------------------------------------------
...
# ----------------------------------------------------------
print("\n=== NETWORK SIM: 50 AGENTS + DYNAMIC PERSONALITIES ===\n")
