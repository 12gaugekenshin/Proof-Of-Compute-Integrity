...
class SettingsWindow(QDialog):
    def __init__(self, parent=None):
        ...
        self.lm_endpoint_edit = QLineEdit(
            getattr(self.cfg, "lm_endpoint", "http://127.0.0.1:1234/v1/chat/completions")
        )
        self.lm_model_edit = QLineEdit(
            getattr(self.cfg, "lm_model", "local-model")
        )
        self.lm_enabled_box = QCheckBox("Use LM Studio for controller input")
        self.lm_enabled_box.setChecked(getattr(self.cfg, "lm_enabled", False))
        ...
        form.addRow("LM Studio Model:", self.lm_model_edit)
        form.addRow("", self.lm_enabled_box)
    ...
    def save_settings(self):
        ...
        # LM Studio fields
        self.cfg.lm_endpoint = self.lm_endpoint_edit.text().strip()
        self.cfg.lm_model = self.lm_model_edit.text().strip() or "local-model"
        self.cfg.lm_enabled = self.lm_enabled_box.isChecked()
